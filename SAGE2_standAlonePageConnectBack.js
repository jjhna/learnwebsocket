...
var SAGE2Connection = {
	...
	afterApplicationStateUpdate: null,
	hostname: null,
	state: {},
	stateLastReceived: null,
	...
	*/
	...
	setupListeners: function() {
		...
		// Only add this listener if the user had created a handler ahead of time
		if (this.afterApplicationStateUpdate) {
			console.log("Adding listener for applicationState and loadApplicationState");
			console.log("TODO make function handler for adding the reaction function for better accounding");
			this.wsio.on('applicationState', function(data) {
				// data has:
				// application, name of app, eg: "googlemaps"
				// id, app_id
				// state, object
				this.state = data.state;
				SAGE2Connection.stateLastReceived = data.state;
				if (SAGE2Connection.afterApplicationStateUpdate
					&& (SAGE2Connection.appId == data.id)) {
						SAGE2Connection.afterApplicationStateUpdate(data.state);
				}
				console.log("applicationState received for app " + data.id + ". Matches this app(" + SAGE2Connection.appId + "): " + (data.id === SAGE2Connection.appId) );
				console.log("Contents for " + data.id, data);
			});
	
			// Receive a message when an application state is updated either remotely or from this page
			this.wsio.on('loadApplicationState', function(data) {
				// Check if different
	
				// TODO need to fill this out to check for value differences and only activate if different
				// May not be possible with objects containing too many sub properties.
				...
				console.log("loadApplicationState received for app " + data.id + ". Matches this app(" + SAGE2Connection.appId + "): " + (data.id === SAGE2Connection.appId) );
				console.log("Contents for " + data.id, data);
			});
		} else {
			console.log("NO applicationState and loadApplicationState handler added");
			console.log("TODO make function handler for adding the reaction function for better accounding");
		}
	...
	*/
	...
	/**
	* Send appilcation state update.
	*/
	SAGE2StateSync: function(stateObject) {
		// Need to update the container better? Currently no generic container
		console.log("WARNING, the container app MUST support master detection");
		// if (this.isMaster) {
		if (true) {
			// Allows non-master updates
			// id: data.id, state: data.remoteState, date: ts
			this.wsio.emit('updateApplicationState', {
				id: this.appId,
				state: stateObject,
				datae: Date.now()
			});
			console.log("erase me, sent updateApplicationState:" + JSON.stringify(stateobj));
		}
	},
...
console.log("Loaded SAGE2Connection file");
